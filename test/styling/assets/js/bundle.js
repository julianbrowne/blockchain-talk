...
function Miner(blockData, targetHashPrefix, updateDisplayFunction) { 
		...
		this.mine = function() { 
			...
			var digest = utils.hash(this.blockData + miner.mineAttempts);

			if(digest.substr(0, miner.targetHashPrefixLength) === this.targetHashPrefix) { 
				updateDisplayFunction(miner.mineAttempts, digest);
				clearTimeout(timer);
...
Reveal.addEventListener('links', function() { 
	...
	var textInputs = $("section.present input.data-entry");
	var formRows = $("section.present .form-row");
	...
	$(".remember-digest").on("click", function() { 
		...
		var newImage = (thisImage === "assets/img/padlock-unlocked.png") ? "assets/img/padlock-locked.png" : "assets/img/padlock-unlocked.png";
...
Reveal.addEventListener("mining", function() { 
	...
	function getAllDigests() { 
		var digestElements = $("section.present .digest");
		var digestList = [];
	...
	function computeTopHash() { 
		var digests = getAllDigests();
		$("#mining-digest").val(BCDemo.utils.hash(digests));
	}
